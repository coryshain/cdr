...
if __name__ == '__main__':
    ...
    argparser.add_argument('-o', '--outdir', default='./gam_data/', help='Output directory')
    ...
    if True:
        ...
        for i in range(len(evaluation_sets)):
            ...
            X = X[0]
            X_cols = [x for x in X.columns if is_numeric_dtype(X[x])]
            for S in range(1, 4):
                for col in X_cols:
                    X[col + '_S%s' % S] = X.groupby(p.series_ids)[col].shift(S).fillna(0)
            Y = Y[0]
