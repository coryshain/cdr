...
def _solve_interpolation(train_points, train_values, order,
  ...
  Args:
    train_points: `[b, n, d]` interpolation c
    enters
  ...
  b, n, d = tf.unstack(tf.shape(train_points), axis=0)
  _, _, k = tf.unstack(tf.shape(train_values), axis=0)
  ...
  with ops.name_scope('construct_linear_system'):
    ...
    if regularization_weight > 0:
      batch_identity_matrix = tf.expand_dims(tf.eye(n), 0)
    ...
    ones = tf.ones([b, n, 1], train_points.dtype)
...
def _apply_interpolation(query_points, train_points, w, v, order):
  ...
  batch_size = tf.shape(query_points)[0]
  ...
  query_points_pad = array_ops.concat([
      query_points,
      tf.ones([batch_size, num_query_points, 1], train_points.dtype)
  ], 2)
