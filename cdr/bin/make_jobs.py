...
if __name__ == '__main__':
    ...
    argparser.add_argument('-j', '--job_types', nargs='+', default=['fit'], help='Type of job to run. List of' + \
                                                                                 ' ``["fit", "predict",' + \
                                                                                 ' "summarize", "plot",' + \
                                                                                 ' "sample", "save_and_exit"]``')
    ...
    argparser.add_argument('-P', '--slurm_partition', default=None, help='Value for SLURM --partition setting,' + \
                                                                         ' if applicable')
    ...
    argparser.add_argument('-s', '--singularity_path', default='', help='Path to singularity image to invoke before' + \
                                                                        ' running')
            ...
            with open(filename, 'w') as f:
                ...
                for job_type in job_types:
                    if job_type.lower() == 'save_and_exit':
                        ...
                        job_str = wrapper % ('python3 -m cdr.bin.predict %s -p %s -m %s %s' % (path,
                                                                                               ' '.join(partitions),
                                                                                               m, cli_args))
