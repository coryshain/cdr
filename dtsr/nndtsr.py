...
class NNDTSR(DTSR):
    ...
    # Arguments
        ...
        optim: A `str` representing the name of the optimizer to use. Choose from `SGD`, `AdaGrad`, `AdaDelta`,
            `Adam`, `FTRL`, `RMSProp`, `Nadam`.
        learning_rate: A `float` representing the initial value for the learning rate.
        learning_rate_decay_factor: A `float` used to compute the rate of learning rate decay (if applicable).
        learning_rate_decay_family: A `str` representing the functional family for the learning rate decay
        ...
        learning_rate_min: A `float` representing the minimum value for the learning rate. If the decay schedule
            would take the learning rate below this point, learning rate clipping will occur.
        loss: A `str` representing the optimization objective. Currently only `MAE` and `MSE` are supported.
    """
