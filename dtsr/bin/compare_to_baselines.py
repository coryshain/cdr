...
if __name__ == '__main__':
    ...
    for task in args.tasks:
        ...
        for m in models:
            p.set_model(m)
            if os.path.exists(p.outdir + '/' + m + '/%s_losses_mse.txt' % args.partition):
                sys_name, trial_name = m.split('_')
                sys_name = model2name(sys_name)
                if sys_name.startswith('LME') and trial_name == 'noRE':
                    l = pd.read_csv(p.outdir + '/' + m + '/%s_losses_mse.txt' % args.partition, sep=' ', header=None, skipinitialspace=True)
                else:
                    l = pd.read_csv(p.outdir + '/' + name2model(m) + '/%s_losses_mse.txt' % args.partition, sep=' ', header=None, skipinitialspace=True)
