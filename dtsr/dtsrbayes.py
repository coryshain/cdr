        ...
        with self.sess.as_default():
            with self.sess.graph.as_default():
                ...
                if not np.isfinite(self.eval_minibatch_size):
                    loss_cur = self.run_loss_op(fd, verbose=False)
                    loss_total += loss_cur
                else:
                    ...
                    for i in range(0, len(time_y), self.eval_minibatch_size):
                        ...
                        loss_total += self.run_loss_op(fd_minibatch, verbose=False)
                    loss_total /= n_eval_minibatch
                loss_total /= self.n_train_minibatch
