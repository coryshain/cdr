...
for cfg in ('brown.ini', 'dundee.ini', 'geco.ini', 'natstor.ini', 'natstormaze.ini', 'provo.ini'):
    for plot in ('', '_surf'):
        for response in ('', '_mu', '_sigma', '_beta'):
            ...
            job_str = 'python3 -m cdr.bin.plot ini_owt/%s -m main -c plot_config%s%s.ini' % (cfg, plot, response)
    ...
    job_str = 'python3 -m cdr.bin.plot ini_owt/%s -m bigram -c plot_config_surf.ini' % cfg
    job_str = wrapper % job_str
    job_str = base % (job_name, job_name) + job_str
    with open(job_name + '.pbs', 'w') as f:
        f.write(job_str)
    ...
    job_str = 'python3 -m cdr.bin.plot ini_owt/%s -m trigram -c plot_config_surf.ini' % cfg
    job_str = wrapper % job_str
    job_str = base % (job_name, job_name) + job_str
    with open(job_name + '.pbs', 'w') as f:
        f.write(job_str)
