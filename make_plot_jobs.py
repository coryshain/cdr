...
for cfg in ('brown.ini', 'dundee.ini', 'geco.ini', 'natstor.ini', 'natstormaze.ini', 'provo.ini'):
    for plot in ('', '_surf'):
        ...
        job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_nosurp -c plot_config%s.ini' % (cfg, plot)
        ...
        job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_gptprob_invert -c plot_config_prob2surp%s.ini' % (cfg, plot)
        job_str = wrapper % job_str
        job_str = base % (job_name, job_name) + job_str
        with open(job_name + '.pbs', 'w') as f:
            f.write(job_str)
        ...
        job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_gptprob_invert -c plot_config%s.ini' % (cfg, plot)
        job_str = wrapper % job_str
        job_str = base % (job_name, job_name) + job_str
        with open(job_name + '.pbs', 'w') as f:
            f.write(job_str)
        ...
        job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_gpt2.00_invert -c plot_config_square2surp%s.ini' % (cfg, plot)
        job_str = wrapper % job_str
        job_str = base % (job_name, job_name) + job_str
        with open(job_name + '.pbs', 'w') as f:
            f.write(job_str)
        ...
        job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_gpt2.00_invert -c plot_config%s.ini' % (cfg, plot)
        job_str = wrapper % job_str
        job_str = base % (job_name, job_name) + job_str
        with open(job_name + '.pbs', 'w') as f:
            f.write(job_str)
        ...
        job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_gptsurpprob -c plot_config%s.ini' % (cfg, plot)
        job_str = wrapper % job_str
        job_str = base % (job_name, job_name) + job_str
        with open(job_name + '.pbs', 'w') as f:
            f.write(job_str)
        ...
        job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_gptpcfg -c plot_config%s.ini' % (cfg, plot)
        job_str = wrapper % job_str
        job_str = base % (job_name, job_name) + job_str
        with open(job_name + '.pbs', 'w') as f:
            f.write(job_str)
        ...
        if cfg.startswith('provo') or surp != 'cloze':
            for suff in ('',):  #, 'prob_h0', '0.50_h0', '0.75_h0', '1.00_h0', '1.33_h0', '2.00_h0'):
                for plot in ('', '_surf'):
                    ...
                    job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_%s -c plot_config%s.ini' % (cfg, surp + suff, plot)
                    job_str = wrapper % job_str
                    job_str = base % (job_name, job_name) + job_str
                    with open(job_name + '.pbs', 'w') as f:
                        f.write(job_str)
                    ...
                    job_str = 'python3 -m cdr.bin.plot ini/%s -m CDR_%s -c plot_config_alls_%s.ini' % (cfg, surp + suff, cfg[:-4])
                    job_str = wrapper % job_str
                    job_str = base % (job_name, job_name) + job_str
                    with open(job_name + '.pbs', 'w') as f:
                        f.write(job_str)
    ...
    for surp in ('gpt',):
        for suff in ('',):
            for plot in ('', '_surf'):
                ...
                job_str = 'python3 -m cdr.bin.plot ini_normal/%s -m CDR_%s -c plot_config%s.ini' % (cfg, surp + suff, plot)
                job_str = wrapper % job_str
                job_str = base % (job_name, job_name) + job_str
                with open(job_name + '.pbs', 'w') as f:
                    f.write(job_str)
